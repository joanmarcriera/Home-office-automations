...
## What it is
Data Copilot is a high-performance, cost-optimized pipeline architecture for converting natural language questions into executable SQL queries. It employs a **Layered Multi-Agent** approach to decompose complex Text-to-SQL tasks into specialized stages. In July 2026, it utilizes **MCP 3.0 Task Protocol** for standardized database discovery and **Claude 4.8 Opus** or **GPT-5.5** for high-fidelity reasoning, while maintaining local-first execution for simpler queries via **Gemma 3** or **Llama 4**.
...
- **Autonomous Error Correction**: Agents using Text-to-SQL to verify their own database-backed task state via **MCP 3.0 Task Protocol**.
...
- **Cost-Optimized Routing**: Routes simple steps to local models (**Gemma 3**) and escalates to frontier models only when needed.
...
## Limitations
- **Sequential Latency**: Multi-agent pipelines introduce more overhead than single-shot prompts, though **FastMCP 3.0** minimizes this.
...
# Initialize the pipeline with MCP 3.0 discovery
pipeline = DataCopilotPipeline(workspace="finance", agent_level="high-confidence")

# Process a natural language question with Task Protocol headers
result = pipeline.execute(
    "Show me transactions over $500 in the last 30 days",
    headers={"X-MCP-Task-Protocol": "3.0"}
)
