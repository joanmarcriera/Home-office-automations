...
It addresses the "VRAM wall" encountered when trying to run high-parameter models (like Llama 3 70B or Mixtral 8x22B) on consumer-grade hardware. By providing ultra-fast inference speeds and flexible quantization, it enables users to fit larger, more capable models into specific memory envelopes (e.g., 24GB or 48GB setups) without the performance penalties often seen in CPU-bound or more generic inference engines.
...
- **Homelab Inference Clusters**: Running distributed inference across multiple mixed-generation NVIDIA GPUs (e.g., a 3090 paired with a 4090).

## Strengths
- **Exceptional Speed**: Often provides the highest tokens-per-second (TPS) for NVIDIA GPUs, frequently exceeding 150+ TPS on 8B models.
...
- **Legacy & Frontier Support**: Optimized kernels for architectures ranging from Pascal (P40) and Ampere (30-series) to Blackwell (B200).
...
## When not to use it
- On non-NVIDIA hardware (use MLX for Mac or llama.cpp for CPU/AMD).
...
# For the latest features (June 2026), install from source
...
```bash
python convert.py \
    -i /models/Llama-3-70B-HF \
    -o /models/working_dir \
    -cf /models/Llama-3-70B-4.5bpw-EXL2 \
    -b 4.5
```
...
config = ExLlamaV2Config("/models/Llama-3-8B-EXL2")
...
- [llama.cpp](llama-cpp.md) — CPU/Cross-platform alternative.
- [vLLM](vllm.md) — Production-grade inference engine.
- [Aphrodite Engine](aphrodite-engine.md) — High-throughput engine based on vLLM.
- [MLX](../infrastructure/mlx.md) — Apple Silicon alternative.
- [Quantization](../../knowledge_base/patterns/quantization.md) — Concept of model compression.
- [NVIDIA Blackwell](https://www.nvidia.com/en-us/data-center/blackwell/) — Frontier hardware support.
- [Flash Attention 2](https://github.com/Dao-AILab/flash-attention) — Core performance optimization.
...
- [LocalLLM Benchmarks (June 2026)](https://github.com/turboderp/exllamav2/discussions)
- [Quantization Guide](../../knowledge_base/patterns/quantization.md)
- [NVIDIA CUDA Documentation](https://docs.nvidia.com/cuda/)
- [Hugging Face EXL2 Models](https://huggingface.co/models?search=exl2)
- [TurboDerp's Patreon (Dev Updates)](https://www.patreon.com/turboderp)
