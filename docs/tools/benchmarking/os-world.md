...
- **Computer Use Research**: Training and assessing agents (e.g., Claude 5.6, GPT-5.6, Qwen 3.6 VL, Gemini 4.0 Ultra) on raw keyboard and mouse control without custom tool-specific APIs.
...
To structure, monitor, and validate desktop observations, FastMCP 3.1 task states, and generated actions programmatically, use strict **Pydantic v2** validation models:
...
class OSWorldObservation(BaseModel):
    ...
    mcp_task: Optional[FastMCPTaskProtocol] = Field(None, alias="mcpTask")
