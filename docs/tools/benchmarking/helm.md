...
## What it is
HELM (Holistic Evaluation of Language Models) is an open-source evaluation framework developed by Stanford University's Center for Research on Foundation Models (CRFM). It is designed to provide a comprehensive, transparent, and multi-dimensional assessment of Large Language Models (LLMs) and Vision-Language Models (VLMs). In July 2026, it stands as the industry-standard academic benchmark for foundation models, including [Gemma 3](../ai_knowledge/local_llms.md) and Claude 4.8.

## What problem it solves
LLM evaluation is often narrow, focusing only on accuracy for a few tasks. HELM addresses this by evaluating models across a wide range of "scenarios" (tasks) and "metrics" (accuracy, fairness, safety, efficiency, etc.). It solves the problem of "performance gaming" by providing a holistic view of model behavior rather than just a single, easily-optimizable score. It also supports the **MCP 3.0 Task Protocol** for evaluating agentic tool-use reliability.
...
- **Agentic Intelligence Testing**: Utilizing **AIR-Bench** (integrated in 2026) to measure multi-step reasoning, [Model Context Protocol](../automation_orchestration/mcp.md) tool-use, and task-oriented autonomy.
...
- **LiteLLM Integration**: HELM v0.7+ supports [LiteLLM](../../services/litellm.md) as a backend, enabling benchmarking of any model compatible with the OpenAI API via a local proxy.
- **Academic Rigor**: Regularly updated by Stanford with new datasets and the latest models (v0.7.x as of July 2026).
...
```bash
# Install the base HELM package (v0.7.x July 2026)
...
# Example: Run a specific scenario programmatically (Advanced)
# Requires defining RunSpec and using the Runner class
```
