...
## When not to use it
- If your workload is entirely English-centric and has already been highly optimized around [Llama 4](../ai_knowledge/llama.md) or [Gemma 3](../ai_knowledge/local_llms.md).
...
class TechnicalSummary(BaseModel):
    tool_name: str = Field(description="The name of the tool described")
    primary_category: str = Field(description="The architecture category")
    core_specifications: list[str] = Field(default_factory=list, description="List of key hardware specs")

try:
    response = client.chat.completions.create(
        model="glm-5.3-instruct",
        messages=[
            {"role": "system", "content": "Extract and output valid JSON conforming strictly to the requested schema."},
            {"role": "user", "content": "GLM-5.3 is a mixture-of-experts model optimized for bilingual code reasoning."}
        ],
        response_format={
            "type": "json_object"
        }
    ...
    print(f"API call to local GLM server failed: {e}")
```
...
- [GLM-4 Announcement Blog](https://open.bigmodel.cn/dev/howuse/glm4)
