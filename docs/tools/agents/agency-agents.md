...
## What it is
Agency-Agents is a comprehensive suite of 110+ specialized AI agent personas designed to transform generic coding assistants into a "complete AI agency." In late September 2026, it serves as a critical configuration and system prompting layer for next-generation IDE-based agents like Claude 5.1, GPT-5.5, and Llama 4, providing them with domain-specific identities, missions, and success metrics through **Native MCP 3.1 support**.
...
- **Native MCP 3.1**: Personas are exposed as Model Context Protocol resources, enabling automated discovery and dynamic injection by compliant agents.
- **Claude 5.1 Optimized**: Persona definitions are optimized with specialized `PreToolUse` and `PostToolUse` logic for high-precision tool calling.
- **Model Agnostic**: Works with any frontier model (including Claude 5.1, GPT-5.5, Llama 4, Gemma 3, and Qwen 3.6).
...
def load_persona(agent_name: str) -> AgentPersona:
    ...
    if not os.path.exists(path):
        # Fallback fake path for demonstration purposes
        dummy_prompt = f"You are {agent_name.replace('-', ' ').title()}, an expert AI agent."
        return AgentPersona(name=agent_name, system_prompt=dummy_prompt)

    with open(path, "r", encoding="utf-8") as f:
        content = f.read()
        return AgentPersona(name=agent_name, system_prompt=content)

# Load the 'reality-checker' persona for validation loops
persona = load_persona("reality-checker")
