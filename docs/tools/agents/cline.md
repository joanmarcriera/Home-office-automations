...
## What it is
Cline (formerly Claude Dev) is an open-source, autonomous AI coding agent that operates natively within VS Code and JetBrains IDEs. It possesses comprehensive access to the local filesystem, terminal, and a built-in browser, enabling it to execute end-to-end software engineering tasks. As of June 2026, Cline is considered the industry standard for open-source, IDE-native agentic development.
...
- **Provider Agnostic**: Robust support for all major LLMs (Claude 4.8 Opus, GPT-5.5, Llama 4) and local models via Ollama.
...
- **Extensible via MCP**: Native support for the Model Context Protocol (MCP) to access external tools like Google Search or specialized APIs.
- **Local First**: Prioritizes local execution and security, with configurable safety guardrails.
...
- If you want to use frontier models like Claude 4.8 Opus with full IDE context without vendor lock-in.
...
- If you are on a very limited token budget and cannot monitor the agent's iterative loops.
...
4. Select `claude-4-8-opus-20260528` as the preferred model for best results.
...
- [Aider](../development_ops/aider.md) (Terminal-centric agent)
