...
## API examples
Roo Code supports custom mode definitions via a `.roomodes` file in the repository root. This python example demonstrates validating a custom mode profile with **Pydantic v2**:
...
class CustomMode(BaseModel):
    slug: str = Field(..., description="Unique URL-friendly identifier for the custom mode")
    name: str = Field(..., description="Display name of the custom mode")
    role_definition: str = Field(..., alias="roleDefinition", description="System prompt defining the role/persona")
    groups: List[str] = Field(..., description="Allowed tool groups, e.g., read, edit, execute, browser")
    custom_instructions: Optional[str] = Field(None, alias="customInstructions", description="Additional guidelines for the mode")

class RooModesConfig(BaseModel):
    custom_modes: List[CustomMode] = Field(..., alias="customModes", description="List of defined custom modes")
