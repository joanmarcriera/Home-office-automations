...
## What it is
Humanizer is a highly optimized, community-driven skill for **Claude Code**, **OpenCode**, and agentic platforms supporting the **MCP 3.1 / FastMCP 3.1** task standards. It is designed to audit, refine, and calibrate AI-generated text, systematically stripping away robotic clichés, "AI-isms," and predictable syntactic patterns to deliver natural, human-like copy.

## What problem it solves
LLM outputs—even from state-of-the-art models like **Claude 5.1**, **GPT-5.5**, and **Gemini 4.0 Pro**—tend to follow specific structural and lexical distributions (e.g., overuse of "pivotal," "testament," "delve," passive voice structures, and overly predictable bulleted summaries). Humanizer solves this by identifying these markers against community-maintained patterns (such as Wikipedia's guidelines on AI writing signs) and applying dynamic style transfers to restore natural rhythm and personality.

## Where it fits in the stack
**Development & Ops / Output Refinement**. Positioned at the **Interaction Layer**, it intercepts raw text generated by agent loops and refines it before it reaches public-facing environments, documentation sites, or customer interfaces.

## Typical use cases
- **Automated Copy Polish**: Post-processing drafts of blog posts, changelogs, and technical documentations to read organically.
...
- **Agentic Slack/Discord Comms**: Refining real-time notifications or reports generated by automated cron jobs before dispatching.
...
- **MCP 3.1 Task Protocol Conformant**: Can be programmatically registered as an automated sub-task in multi-agent pipelines.
...
## Limitations
- **Model-Quality Dependence**: Relies on advanced frontier models (e.g., Claude 5.1 Sonnet or GPT-5.5) for sophisticated contextual tone shifting.
...
export async function humanizeContent(rawOutput: str): Promise<string> {
  const systemInstruction = `
    You are a professional editor. Apply the Humanizer v3.1 protocol to the user's input.
    - Remove AI-isms ('delve', 'testament', 'pivotal', 'in conclusion').
