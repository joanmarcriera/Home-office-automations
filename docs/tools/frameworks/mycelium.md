...
## What it is
Mycelium is an enterprise-grade, Clojure-based framework and architectural pattern for building robust, observable AI systems using state machines and formal contracts. As of early 2027, it is the primary implementation of the 'Cellular Agent Architecture', where complex multi-agent reasoning workflows are decomposed into isolated, functional 'cells' that communicate exclusively via strongly-typed Malli schemas and support **FastMCP 3.1** dynamic tool invocation with **Claude 5.6**, **GPT-5.6**, and **Gemini 4.0 Ultra**.
...
**Orchestration / Control Plane**. Mycelium sits above the LLM inference layer (LiteLLM, Claude 5.6, GPT-5.6, Gemini 4.0 Ultra, DeepSeek-V4, Llama 4, and Gemma 4) and acts as the supervisor for agentic state. It is used as the logical backbone for enterprise environments where reliability, strict verification, and deep telemetry are prioritized.

## Typical use cases
- **Multi-Agent Coding Factories**: Coordinating specialized agents for planning, implementation, and testing with strict handoff protocols and contract-based verification.
...
- **MCP-Orchestrated Environments**: Dynamically routing tasks from Model Context Protocol (FastMCP 3.1) clients to specialized Clojure cells for secure execution.
...
- **Agent-Friendly Structure**: The explicit structure provides clear cues that improve frontier model performance (e.g., Claude 5.6, GPT-5.6).
...
## Limitations
- **Functional Paradigm**: Requires proficiency in Clojure and functional programming, which may pose a learning curve for Python-native AI teams.
...
- When rapid Python-native prototyping is prioritized over long-term system stability and formal verification.

## Getting started

### Installation (deps.edn)
Add the stable release to your Clojure project:
```clojure
...
(def CodeAuditCell
  {:id :code-audit
   ...
   :fn (fn [{:keys [code standard]}]
         ;; Logic to call Claude 5.6 via LiteLLM or an FastMCP 3.1 server
...
Mycelium nodes are started via the Clojure CLI or as part of an Uberjar deployment.
```bash
...
Mycelium ensures that data passing between agents (cells) is always valid using Malli schemas, which provide runtime verification similar to Python's Pydantic v2.
...
;; and automatically triggers a 'Self-Correction' cell if validation fails.
(m/register-cell!
  {:id :agent-dispatcher
   :output-schema AgentContract
   :fn (fn [in]
         ;; Calls frontier models (e.g., Claude 5.6) and returns mapped result
         (let [raw-response (m/llm-call :claude-5-6 (m/build-prompt in))]
           ;; Output will be strictly validated against output-schema (AgentContract) by Mycelium
           raw-response))})
```

### Model Context Protocol (FastMCP 3.1) Client Integration
Mycelium cells interact directly with FastMCP 3.1 servers to discover and consume tools in real-time.
```clojure
...
- **Workflow**: n8n receives a webhook -> forwards payload to Mycelium API -> Mycelium runs state machine logic -> Mycelium sends command back to n8n for execution (e.g., Slack, Email).
