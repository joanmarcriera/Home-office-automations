...
3. **Draft your Prompt**: Use the [How to instruct the LLM](#how-to-instruct-the-llm) section to generate your code using **Claude 5.1/5.6** or **GPT-5.5/5.6**.
4. **Deploy via CLI**: Use the provider's CLI to deploy your site.

### Model Context Protocol (MCP) Integration
As of early 2027, using **Claude 5.1/5.6**, **GPT-5.5/5.6**, or **Gemini 4.0** with **MCP 3.1** servers is the recommended way to scaffold and manage these sites.
...
### Vercel AI SDK (v5.1) with Claude 5.6
...
export async function POST(req: Request) {
  ...
  const result = await streamText({
    model: anthropic('claude-5-6-sonnet-20270105'),
    messages,
  });
