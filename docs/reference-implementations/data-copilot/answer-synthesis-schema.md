...
## What it is
Data Copilot Answer Synthesis (v3.2, November 2026) is a Pydantic-based schema and prompt contract for the final stage of an agentic data pipeline. It ensures that every response generated by models like [Gemma 3](../../tools/ai_knowledge/local_llms.md) or Claude 5.1 includes underlying reasoning, specific source citations, confidence scores, and recommended next steps, now fully supporting **MCP 3.1** and **FastMCP 3.1** structured outputs.
...
Initialize the Pydantic model with your LLM's JSON output to ensure type safety and validation. This implementation utilizes **Pydantic v2** conventions.
...
# Initialize synthesizer with MCP 3.1 support
