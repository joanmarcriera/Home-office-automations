...
## What it is
Mealie is a comprehensive culinary management system that allows users to import recipes from the web, organize their collection, create meal plans, and generate shopping lists in a centralized, private environment. As of June 2026, it features native integration with **Claude 4.8 Opus** for AI-powered recipe scaling and nutrition analysis.
...
**Service / Home Automation**. It sits in the **personal lifestyle and planning** layer of the self-hosted stack, connecting culinary interests with inventory management and grocery logistics. It can be integrated with [Grocy](grocy.md) for deeper pantry management and [n8n](n8n.md) for automated grocery list syncing.
...
- **AI Ingredient Extraction**: Using GPT-5.5 or Claude 4.8 Opus to extract ingredients from unstructured text or voice notes.
...
- **VRAM for Local AI**: If using local whisper models for video transcription, significant GPU resources are required.
...
Mealie v3.18.0 (June 2026) supports both SQLite and PostgreSQL backends.
...
Mealie v3.18.0 utilizes a two-tier user model for multi-tenant or multi-family deployments:
...
- **Backend**: Transcribes video and structures ingredients/steps using GPT-5.5.
...
- [Whisper](whisper.md) — for local video-to-text transcription used in recipe imports.
...
- [Mealie Release Notes](https://github.com/mealie-recipes/mealie/releases)
