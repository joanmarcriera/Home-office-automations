...
## What it is
Homebox is a lightweight, self-hosted inventory management system written in Go. It uses a single SQLite database for all data, making it extremely easy to host, migrate, and backup. It is designed for home users to catalog physical assets across multiple locations.

## What problem it solves
It centralizes the tracking of household items, warranties, and insurance information, replacing disorganized spreadsheets or physical receipts with a searchable, location-aware digital catalog. It ensures you have a detailed record of your belongings for insurance claims or organizational purposes.

## Where it fits in the stack
It is a **Standalone Service** in the home automation stack, typically deployed via Docker and accessible via a web browser. It acts as the structured metadata layer for physical assets.
...
- **Agentic Asset Retrieval**: Using Claude 4.8 Opus to query the SQLite database via Python to locate specific tools or parts.

## Strengths
- **Lightweight**: Minimal CPU and RAM footprint, suitable for low-powered hardware.
- **Fast**: Highly responsive web interface and search.
- **Portable**: SQLite backend makes backups and environment moves trivial.
- **v0.25+ Features**: Enhanced tag relationships (parent/child), fractional quantities, and OpenTelemetry support for observability.

## Limitations
- **Simplicity**: Lacks advanced supply chain or POS features found in enterprise ERP systems.
- **Permissions**: Limited multi-user role-based access control (RBAC).
...
Since Homebox uses SQLite, standard CLI tools can be used for maintenance.
...
### Binary Information
Accessing version and help directly from the container.
...
### Health Check (Curl)
Simple automated health monitoring for n8n or uptime agents.

```bash
curl -X GET "http://localhost:3100/api/v1/health"
```

### Fetching Items (Python)
Programmatic access for custom reporting or AI agent integration.
...
- [Immich](immich.md) — For high-performance photo storage of assets.
