...
- **Agentic Journaling**: Using scripts to automatically summarize daily logs via Claude 4.8 Opus.
...
- **Legacy Scripting**: Older scripts using `api.axios` must be migrated to the native `fetch()` API in newer versions.
...
Utilize GPT-5.5 or Claude 4.8 Opus to orchestrate note creation via the REST API.
...
note_data = {
    ...
    "type": "text",
    "content": "Content generated by Claude 4.8 Opus."
}
...
TriliumNext v0.103+ has transitioned to native `fetch`.
